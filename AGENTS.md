...
- **Lefthook Auto-Apply:** A pre-commit hook in `lefthook.yml` automates running `tofu apply -auto-approve` whenever `.tf` files are staged, running only if Cloudflare API credentials are set in the shell session.
